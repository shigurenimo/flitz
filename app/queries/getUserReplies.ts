...
import { CountRepliesByUsernameQuery, FindPostRepliesQuery } from "service"
...
const getUserReplies = resolver.pipe(
  ...
  async (props) => {
    const query = container.resolve(FindPostRepliesQuery)
    ...
    const countQuery = container.resolve(CountRepliesByUsernameQuery)
